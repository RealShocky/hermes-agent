...
:::caution Minimum context: 32K tokens
Hermes Agent requires a model with at least **32,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 32K (e.g. `--ctx-size 32768` for llama.cpp or `-c 32768` for Ollama).
:::
